...
parser.add_argument(
    '--epochs_per_eval', type=int, default=1000,
...
def filenames(is_training, data_dir):
  ...
  if is_training:
    return [
        os.path.join(data_dir, 'photo_test')]
  else:
    return [
...
def record_parser(value, is_training):
  ...
  image = tf.image.decode_image(
      tf.reshape(parsed['image_raw'], shape=[]),
      _NUM_CHANNELS)
  '''
  image = tf.decode_raw(parsed['image_raw'], tf.uint8)
  image = tf.reshape(image, [256, 256, _NUM_CHANNELS])
  ...
  label = tf.cast(
      tf.reshape(parsed['image_label'], shape=[]),
      dtype=tf.int32)
